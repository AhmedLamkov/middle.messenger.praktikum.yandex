...
export default class RegisterPage extends Block {
	...
	private ValidateEmail (e: Event) {
		...
		const email = this.children.email;
	...
	private ValidatePassword(e: Event) {
		...
		const confirmPasswordElement = confirmPassword.element?.querySelector('input') as HTMLInputElement;
	...
	private ValidatePhone(e: Event) {
		const target = e.target as HTMLInputElement;
		const value = target.value.trim();
		const phone = this.children.phone;
		let error = "";
